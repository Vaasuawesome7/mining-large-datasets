...
HashTable1 = [0] * constants.HASH_TABLE_SIZE_MS1
HashTable2 = [0] * constants.HASH_TABLE_SIZE_MS2
    ...
    for basket in baskets:
        for i in range(len(basket)):
            for j in range(i+1, len(basket)):
                ...
                HashTable1[constants.hash_function_ms1(items=mitems)] += 1
    ...
    for basket in baskets:
        for i in range(len(basket)):
            for j in range(i+1, len(basket)):
                ...
                mitems = [item1, item2]
                if L1.get(item1) != None and L1.get(item2) != None and HashTable1[constants.hash_function_ms1(items=mitems)]:
                    HashTable2[constants.hash_function_ms2(mitems)] += 1

def intermediate_pass_2_3():
    global HashTable2
    ...
    HashTable2 = TempHashTable

def third_pass(baskets: list[list], itemIndexTable: dict,  L1: dict):
    items = list(L1.keys())
    ...
    for i in range(len(items)):
        for j in range(i+1, len(items)):
            ...
            if HashTable1[constants.hash_function_ms1(mitems)] == True and HashTable2[constants.hash_function_ms2(mitems)] == True:
...
def fourth_pass(baskets: list[list], itemIndexTable: dict, L2: dict):
    keys = list(L2.keys())
    C3 = {}
